-- 098_event_attributed_purchase_orders.sql
-- E4: event-attributed purchasing. A purchase order can be linked to a
-- confirmed event/function booking (bookings.booking_id) so the event's
-- purchasing is traceable per event; an optional PO-level cost centre
-- (cost_centre_id) lets receiving cost the goods to the event's centre
-- (Events / Private Functions / VIP Room) instead of the location default.
--
-- Part 1 (additive, preserving all existing data):
--   - inventory_purchase_orders.booking_id   UUID NULL REFERENCES bookings(id)
--   - inventory_purchase_orders.cost_centre_id UUID NULL REFERENCES cost_centres(id)
--   - Historical POs stay NULL (plain replenishment) - nothing rewritten.
--
-- Part 2: replays receive_purchase_order (089's 8-arg signature, migration
-- history immutable - same pattern as 090/091) so the per-line cost centre
-- resolution becomes:
--   1. Explicit receive-time p_cost_centre_id (validated) - wins
--   2. The PO's own cost_centre_id (event centre, validated at creation) 
--   3. The line's location cost centre (previous behaviour)
-- P1a identity / P1b over-receive / P1c shortage reasons / P1d auto-invoice /
-- P1e due dates unchanged (byte-for-byte from 089).

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema = 'public' AND table_name = 'inventory_purchase_orders'
                   AND column_name = 'booking_id') THEN
    ALTER TABLE public.inventory_purchase_orders ADD COLUMN booking_id UUID REFERENCES bookings(id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema = 'public' AND table_name = 'inventory_purchase_orders'
                   AND column_name = 'cost_centre_id') THEN
    ALTER TABLE public.inventory_purchase_orders ADD COLUMN cost_centre_id UUID REFERENCES cost_centres(id);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_po_booking_id ON public.inventory_purchase_orders(booking_id)
  WHERE booking_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_po_cost_centre_id ON public.inventory_purchase_orders(cost_centre_id)
...
BEGIN
  ...
  -- P1e: supplier payment term rides along for the due-date calculation.
  -- E4: the PO's own cost centre rides along for event-attributed ordering.
  SELECT po.status, po.supplier_id, s.payment_term_type, s.payment_term_days, po.cost_centre_id
    INTO v_po_status, v_supplier_id, v_term_type, v_term_days, v_po_cost_centre
  ...
  FOR v_el IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Cost centre (E4): explicit receive-time override wins, then the PO's
    -- own cost centre (event-attributed ordering), then the location's.
    ...
    ELSIF v_po_cost_centre IS NOT NULL THEN
      IF NOT EXISTS (SELECT 1 FROM public.cost_centres WHERE id = v_po_cost_centre AND is_active) THEN
        RAISE EXCEPTION 'Cost centre % does not exist or is not active', v_po_cost_centre;
      END IF;
      v_cc := v_po_cost_centre;
    ELSE
  ...
  -- P1d + P1e: auto-create the supplier invoice inside the same transaction.
  -- One receipt -> one invoice (unique index on receipt_id). Due date comes
  -- from the supplier's structured payment term (never historical invoices).
  v_due_date := CASE
    WHEN v_term_type = 'WEEKLY' THEN CURRENT_DATE + 7
    WHEN v_term_type = 'MONTHLY' THEN (CURRENT_DATE + interval '1 month')::date
    WHEN v_term_type = 'ACCOUNT' THEN CURRENT_DATE + COALESCE(v_term_days, 30)
    ELSE CURRENT_DATE
  END;

  INSERT INTO public.inventory_supplier_invoices
    (supplier_id, receipt_id, invoice_number, invoice_date, due_date, total_amount, status, notes, created_by)
  VALUES
    (v_supplier_id, v_receipt_id, p_invoice_number, CURRENT_DATE, v_due_date, v_invoice_total, 'pending',
