-- 072_combined_dashboard.sql
-- Ingress fix: collapse the /api/inventory/dashboard?section=combined route from
-- ~17 separate PostgREST round-trips per poll down to a single RPC call.
--
-- PostgREST aggregates are disabled on this project, so aggregation lives here
-- (same pattern as migration 070/071 stock-sheet RPCs). The function replicates
-- the exact engine semantics of src/inventory/engine/dashboard.ts combined:
--
--   summary        today buckets, count of active products, low-stock count
--                  (products with reorder_threshold set), 90-day inventory value
--   alerts         per-product balance (full ledger, location-scoped)
--   recent         last 10 transactions w/ product names (location-scoped)
--   fastMovers     top 5 by 30-day sale quantity (location-scoped)
--   slowMovers     bottom 5 among active products (location-scoped)
--   value          same as summary.inventoryValue (90-day window)
--   todayTxnns     today's per-type counts + signed totals (location-scoped)
--   purchaseOrders open/overdue counts + up to 5 overdue and 5 recent POs
--
-- Bug-for-bug parity notes (kept identical to the engine):
--   - summary's today buckets scan ALL locations (engine omits location filter);
--     the "today" section below IS location-scoped, matching getTodayTransactions.
--   - lowStockCount counts threshold-flagged products, not actually-low ones.
--   - Name lookups use all products (including archived); 'Unknown' fallback.
--   - overdueCount is the capped returned-array length (max 5), like the engine.
--
-- security definer + search_path so the service-role caller works regardless of
-- RLS; p_location NULL = all locations (route always resolves a location).
...
select jsonb_build_object(
  'summary', jsonb_build_object(
    ...
    'lowStockCount', (select count(*)::int from prods where reorder_threshold is not null),
    'outOfStockCount', 0,
