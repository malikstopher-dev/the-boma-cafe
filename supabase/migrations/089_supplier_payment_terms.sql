-- 089_supplier_payment_terms.sql
-- P1e: structured payment terms on suppliers + due dates on auto-created
-- invoices (overdue is computed at read time - no scheduler).
--
-- Part 1 (additive, preserving all existing data):
--   - payment_term_type TEXT  : NULL | CASH | COD | ACCOUNT | WEEKLY | MONTHLY
--   - payment_term_days INT   : NULL | >= 0  (used when type = ACCOUNT)
--   - Existing suppliers stay valid (NULL type -> CASH semantics at read time:
--     due on the invoice date). Existing invoices are NEVER rewritten.
--   - Backfill obvious free-text values from the legacy payment_terms column
--     ("Weekly" -> WEEKLY, etc.; a number in an Account/credit string sets
--     payment_term_days).
--
-- Part 2: replays receive_purchase_order (088's 8-arg signature) to compute
-- the invoice due date at creation time from the supplier's structured term:
--   CASH/COD/NULL -> due today; WEEKLY -> +7 days; MONTHLY -> same day next
--   month (Postgres clamps month ends, e.g. Jan 31 -> Feb 28);
--   ACCOUNT -> + COALESCE(payment_term_days, 30) days.
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema = 'public' AND table_name = 'inventory_suppliers'
                   AND column_name = 'payment_term_type') THEN
    ALTER TABLE public.inventory_suppliers ADD COLUMN payment_term_type TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema = 'public' AND table_name = 'inventory_suppliers'
                   AND column_name = 'payment_term_days') THEN
    ALTER TABLE public.inventory_suppliers ADD COLUMN payment_term_days INT;
  END IF;
END $$;

ALTER TABLE public.inventory_suppliers
  DROP CONSTRAINT IF EXISTS suppliers_payment_term_type_check;
ALTER TABLE public.inventory_suppliers
  ADD CONSTRAINT suppliers_payment_term_type_check
  CHECK (payment_term_type IS NULL OR payment_term_type IN ('CASH', 'COD', 'ACCOUNT', 'WEEKLY', 'MONTHLY'));

ALTER TABLE public.inventory_suppliers
  DROP CONSTRAINT IF EXISTS suppliers_payment_term_days_check;
ALTER TABLE public.inventory_suppliers
  ADD CONSTRAINT suppliers_payment_term_days_check
  CHECK (payment_term_days IS NULL OR payment_term_days >= 0);

-- Backfill obvious values from the legacy free-text column.
UPDATE public.inventory_suppliers
   SET payment_term_type = CASE
         WHEN payment_terms ILIKE '%week%' THEN 'WEEKLY'
         WHEN payment_terms ILIKE '%month%' THEN 'MONTHLY'
         WHEN payment_terms ILIKE '%cash on delivery%' OR payment_terms ILIKE '%cod%' THEN 'COD'
         WHEN payment_terms ILIKE '%cash%' THEN 'CASH'
         WHEN payment_terms ILIKE '%account%' OR payment_terms ILIKE '%credit%' THEN 'ACCOUNT'
         ELSE payment_term_type
       END,
       payment_term_days = CASE
         WHEN (payment_terms ILIKE '%account%' OR payment_terms ILIKE '%credit%')
              AND payment_term_days IS NULL
         THEN COALESCE((regexp_match(payment_terms, '\d+'))[1]::INT, 30)
...
BEGIN
  ...
  SELECT po.status, po.supplier_id, s.payment_term_type, s.payment_term_days
    INTO v_po_status, v_supplier_id, v_term_type, v_term_days
