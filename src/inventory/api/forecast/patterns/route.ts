...
import { getConsumptionPattern } from '@/inventory/engine/forecasting'
...
export async function GET(request: NextRequest): Promise<NextResponse<ApiResponse<unknown>>> {
  try {
    ...
    const days = Math.min(Math.max(Number(searchParams.get('days')) || 60, 7), 180)
    ...
    const data = await getConsumptionPattern(locationId, days, inventoryType ?? undefined)
