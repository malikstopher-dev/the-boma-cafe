...
import { getPurchaseOrder } from '@/inventory/engine/purchase-orders'
...
export async function POST(
  ...
  try {
    ...
    // The receive RPC is the only write path. A schema, permission or
    // business error must remain visible; downgrading to the old engine loop
    // would discard the atomicity guarantee precisely when the RPC fails.
    ...
    if (rpcRes.error || !rpcRes.data) {
      throw new Error(`Purchase-order receive failed atomically: ${rpcRes.error?.message ?? 'no result returned'}`)
    }
