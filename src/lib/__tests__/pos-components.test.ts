...
// Run: npx tsx src/lib/__tests__/pos-components.test.ts
...
test('getUrgencyColor: <70% → green', () => {
  const result = getUrgencyColor(500000, 15) // 500s / 900s = 55%
  assert.equal(result.color, '#10b981')
})

test('getUrgencyColor: 70-100% → orange', () => {
  const result = getUrgencyColor(700000, 15) // 700s / 900s = 77%
  assert.equal(result.color, '#f59e0b')
})

test('getUrgencyColor: >100% → red', () => {
  const result = getUrgencyColor(1000000, 15) // 1000s / 900s = 111%
  assert.equal(result.color, '#ef4444')
})

test('getUrgencyColor: exactly 70% → orange', () => {
  const result = getUrgencyColor(630000, 15) // 630s / 900s = 70%
  assert.equal(result.color, '#f59e0b')
})

test('getUrgencyColor: exactly 100% → red', () => {
  const result = getUrgencyColor(900000, 15) // 900s / 900s = 100%
  assert.equal(result.color, '#ef4444')
})

test('getUrgencyColor: bar target 5min', () => {
  const green = getUrgencyColor(100000, 5) // 100s / 300s = 33%
  const orange = getUrgencyColor(200000, 5) // 200s / 300s = 66%
  const red = getUrgencyColor(350000, 5) // 350s / 300s = 116%
  assert.equal(green.color, '#10b981')
  assert.equal(orange.color, '#10b981')
...
type OrderStatus = 'pending' | 'confirmed' | 'preparing' | 'packing' | 'ready' | 'served' | 'completed' | 'cancelled' | 'rejected'
...
const ALL_STATUSES: OrderStatus[] = ['pending', 'confirmed', 'preparing', 'packing', 'ready', 'served', 'completed', 'cancelled', 'rejected']
...
test('All 3 order types have required fields', () => {
  const types: OrderType[] = ['dine-in', 'delivery', 'pickup']
...
function countDisplay(count: number, max: number = 99): string {
  if (count <= 0) return ''
...
test('All 6 variants have color and bg', () => {
  ...
  for (const v of variants) {
    const style = (VARIANT_STYLES as any)[v]
