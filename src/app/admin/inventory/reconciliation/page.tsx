...
export default function ReconciliationPage() {
  ...
  const fetchReconciliation = useCallback(async () => {
    setIsLoading(true)
    try {
      const res = await fetch(`/api/inventory/dashboard?section=reconciliation&location_id=main&date=${date}`)
  ...
  async function savePhysical(productId: string) {
    ...
    try {
      ...
      // Create a stock count item via the existing API for recording physical counts
      // For V1, just mark it locally
      setRows(prev =>
        prev.map(r =>
  ...
  return (
    <AdminPage
      title="Morning Reconciliation"
      subtitle="Compare expected vs actual stock levels"
      actions={
        <div className="flex gap-2">
          ...
          />
          <Button onClick={fetchReconciliation} variant="secondary" size="sm">Refresh</Button>
        </div>
      }
    >
      <div className="p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-white mt-1">
              {rows.filter(r => physMap[r.productId] !== undefined || r.physicalQuantity !== null).length}
                    ...
                    <tr key={row.productId} className="border-b border-gray-800 hover:bg-gray-800/30">
                      ...
                      <td className="py-2 px-3">
                        <input
                          ...
                          className="w-24 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-right text-white text-sm text-right ml-auto"
                          placeholder="0"
                        />
                      </td>
