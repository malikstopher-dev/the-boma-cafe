...
- Batch 3 route-level RBAC/privacy is **IMPLEMENTED LOCALLY - AWAITING DEPLOYMENT APPROVAL** (2026-08-27). Owner/full_manager-only permission keys now gate CMS/settings/pricing/media/background jobs; pricing/settings/manual-job inputs are allowlisted; background-job DTOs are redacted and mutations audited; public CMS settings and staff login DTOs are minimized; voice uploads require membership, validated WebM bytes, private storage, and signed reads; push tokens are atomically bound to server-resolved identity. Migration 116 is pending and has not been applied. Local evidence: 488/488 tests, inventory/root TypeScript, 187-page Next build, linked dry run listing only migration 116, and schema lint with only pre-existing warnings. No Batch 3 commit, push, deployment, production mutation, or worker restart has occurred.
