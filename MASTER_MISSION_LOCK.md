...
- Batch 2 H-08 through H-14 was subsequently owner-activated and reached the local gate on 2026-08-27. Migrations 112-115 and their callers/tests are implemented locally: atomic ledger/audit/cache, fail-closed stock-count reads, no arbitrary RPC downgrade, locked/capped/idempotent supplier payments with owner/full_manager finance permissions, worker lease fencing/provider-idempotent notification outbox, and stable order-line reconciliation.
- Batch 2 local evidence: 471/471 inventory tests; inventory/root TypeScript; 105.06 KB worker bundle; migration dry-run listing exactly 112-115; linked lint with only the pre-existing `v_rows` warning; diff check; and a complete 187-page Next build. Production cutover remains approval-gated: no Batch 2 migration, commit, push, Vercel deploy, worker restart, provider call, or production business-row mutation has occurred.
